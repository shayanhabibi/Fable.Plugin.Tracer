...
module TracerConfiguration =
    let [<Literal>] private minimal = "MINIMAL"
    let [<Literal>] private debug = "DEBUG"
    let [<Literal>] private trace = "TRACE"
    let [<Literal>] private file = "FILE"
    ...
    let (|ContainsSomethingStartingWith|_|) (value: string) (input: string list) = input |> List.tryFind (_.StartsWith(value))
    /// <summary>
    ...
    type Settings() =
        ...
        static let StandardOutput = let this = new IO.StreamWriter(Console.OpenStandardOutput()) in this.AutoFlush <- true; this
        ...
        static member configure (pluginName: string) (helper: Fable.PluginHelper) =
            ...
            if helper.Options.Define |> List.exists (fun definition -> definition.StartsWith $"{PluginName}_") then
                Verbose <- true
                let definitions =
        ...
        static member registerConverter<'T> () =
            JsonConverters.UnionConverter<'T>()
            |> Settings.addConverter
type Tracer =
    static member ping(
...
type Tracer<'T> = {Value: 'T; Guid: Guid; ConsoleColor: ConsoleColor} with
    member private this.emit (message: string, memberName: string, path: string, line: int, [<Optional;DefaultParameterValue(true)>] emitJson: bool) =
        let print () =
        ...
        if Settings.verbose() then
            ...
            if Settings.jsonify() && emitJson then
                if Settings.toFile() then
                    Settings.canPipe()
                    print()
                    Console.WriteLine $"{PrettyPrinter.print this.Value}" // TODO add pretty print
                    Settings.closePipe()
                else
                    Console.WriteLine $"{PrettyPrinter.print this.Value}" // TODO add pretty print
    ...
    member this.trace(
            ...
            [<CallerFilePath; Optional; DefaultParameterValue("")>] path: string,
            [<CallerLineNumber; Optional; DefaultParameterValue(0)>] line: int
        ) = this.emit (message, memberName, path, line); this
    member this.ping(
            [<Optional; DefaultParameterValue("")>] message: string,
            [<CallerMemberName; Optional; DefaultParameterValue("")>] memberName: string,
            [<CallerFilePath; Optional; DefaultParameterValue("")>] path: string,
...
module Tracer =
    ...
    let map (func: 'T -> 'M) (tracer: Tracer<'T>) =
        func tracer.Value |> bind tracer
...
module JsonConverters =
    ...
    let (|IsUnion|_|) (input: 'T) = let typ = typeof<'T> in if FSharpType.IsUnion typ then FSharpValue.GetUnionFields(input, typ) |> Some else None
    ...
    type UnionConverter<'T>() =
        ...
        override this.Write(writer, value, options) =
            ...
            | IsUnion (unionValue,unionFields) ->
                ...
                | NoFields ->
                    writer.WriteRawValue(sprintf "\"%s\"" unionName, true )
                    // Union name is value
                | SingleField field ->
...
type PrettyPrinter =
    static member print(input: 'T): string = JsonSerializer.Serialize(input, Settings.converterOptions())
